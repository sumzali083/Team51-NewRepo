...
export function NewArrivalsPage() {
    ...
    useEffect(() => {
        // Get products from each category and mix them for better visual appearance
        const menProducts = PRODUCTS.filter(p => p.cat === "men").slice(0, 2);
        const womenProducts = PRODUCTS.filter(p => p.cat === "women").slice(0, 2);
        const kidsProducts = PRODUCTS.filter(p => p.cat === "kids").slice(0, 2);

        // Mix products: men[0], women[0], kids[0], men[1], women[1], kids[1]
        const mixed = [
            menProducts[0],
            womenProducts[0],
            kidsProducts[0],
